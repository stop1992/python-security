...
def main():
    """
    @function Main function of penework when running from command line.
    """
    pcsInit()
    ...
    try:
        ...
        if argsDict['dork']:
            ...
            if z.newToken():
                ...
                if z.newToken():
                    ...
                    sys.exit(logger.log(CUSTOM_LOGGING.ERROR, 'ZoomEye API authorization failed, make sure correct credentials provided in "~/.peneworkrc".'))
        ...
        if argsDict['vulKeyword']:
            ...
            if (s.token or not s.static()) or not s.token:
                    ...
                    s.write_conf()
                    if not s.static():
                        sys.exit(logger.log(CUSTOM_LOGGING.ERROR, 'Seebug API authorization failed, make sure correct credentials provided in "~/.peneworkrc".'))
